...
const script = [
  ...
  ['Remove token A now.', p => {
    ...
    assert(c(p).response_intent === 'main_teach', `turn 2: expected main_teach; got ${JSON.stringify(c(p).response_intent)}`);
    assert(c(p).pending_gap != null, 'turn 2: teach moment must be grounded in a real pending gap');
  }],
  ['Keep removing one token at a time. Next remove token B. The game ends when no available tokens are left.', p => {
    assert(p.phase === 'practice', `turn 3: repair should enter practice; got ${JSON.stringify(p.phase)}`);
    assert(p.support?.type === 'game_guide' && p.support?.mode === 'compact', `turn 3: guide should become compact; got ${JSON.stringify(p.support)}`);
    assert(actions(p).some(a => a.type === 'remove_object'), 'turn 3: B must be removed');
  }],
  ['Remove token C now.', p => {
    assert(p.phase === 'practice', `turn 4: same game must continue; got ${JSON.stringify(p.phase)}`);
    assert(p.support?.type === 'game_guide' && p.support?.mode === 'compact', `turn 4: compact guide should remain once; got ${JSON.stringify(p.support)}`);
    assert(c(p).scaffold_fade_step === 1, `turn 4: expected fade step 1; got ${JSON.stringify(c(p).scaffold_fade_step)}`);
    assert(actions(p).some(a => a.type === 'remove_object'), 'turn 4: C must be removed');
  }],
  ['Remove token D now.', p => {
    assert(p.phase === 'practice', `turn 5: same game must continue; got ${JSON.stringify(p.phase)}`);
    assert(p.support == null, `turn 5: persistent guide should fade away; got ${JSON.stringify(p.support)}`);
    assert(c(p).scaffold_faded === true, 'turn 5: scaffold_faded must be true');
    assert(actions(p).some(a => a.type === 'remove_object'), 'turn 5: D must be removed');
  }],
  ['Remove token E now.', p => {
    assert(p.phase === 'transfer', `turn 6: grounded ending should go directly to transfer; got ${JSON.stringify(p.phase)}`);
    assert(p.support == null, 'turn 6: no guide at transfer');
    assert(actions(p).some(a => a.type === 'remove_object'), 'turn 6: final physical action must execute');
    assert(c(p).game_complete === true && (c(p).completion_evidence || []).length > 0, `turn 6: grounded completion evidence required; controller=${JSON.stringify(c(p))}`);
    assert(c(p).lesson_complete === false, 'turn 6: game end is not lesson end');
    assert(c(p).response_intent === 'transfer_prompt', `turn 6: expected transfer_prompt; got ${JSON.stringify(c(p).response_intent)}`);
  }],
  ['what?', p => {
    assert(p.phase === 'transfer', `turn 7: trivial response should stay transfer; got ${JSON.stringify(p.phase)}`);
    assert(actions(p).length === 0, 'turn 7: transfer must not mutate gameplay');
    assert(c(p).response_intent === 'transfer_retry', `turn 7: expected transfer_retry; got ${JSON.stringify(c(p).response_intent)}`);
  }],
  ['Next time I will first check what the other player already knows, then explain what they still need before the next move.', p => {
    assert(p.phase === 'complete', `turn 8: substantive transfer should complete; got ${JSON.stringify(p.phase)}`);
    assert(actions(p).length === 0, 'turn 8: completion must not mutate gameplay');
    assert(c(p).lesson_complete === true && c(p).response_intent === 'lesson_complete', `turn 8: lesson_complete required; controller=${JSON.stringify(c(p))}`);
  }],
];
...
try {
  ...
  for (let i = 0; i < script.length; i += 1) {
    ...
    assert(payload?.debug?.dsl_version === expectedDslVersion, `turn ${turn}: DSL mismatch; expected ${expectedDslVersion}, got ${JSON.stringify(payload?.debug?.dsl_version || null)}`);
